...
def redact_content(content, matches):
    ...
    for start, end, _, _ in matches:
        length = end - start
        redacted_content[start:end] = ['█'] * length
...
def create_post_widget(parent_frame, post):
    ...
    if post.get('is_flagged'):
        content_display = "⚠️ [AUTO-REDACTED] " + content_display
    ...
    like_btn_text = "❤ Liked" if is_liked else "🤍 Like"
...
if __name__ == "__main__":
    ...
    tk.Button(header_frame, text="Post ✍", command=open_post_editor, bg=ACCENT_COLOR, fg=TEXT_COLOR, font=FONT_BOLD, relief=tk.FLAT).pack(side=tk.RIGHT) 
